...
// 주의: 이 파일은 app.bundle.js보다 먼저 불러와야 합니다.
      ...
      return new Promise(resolve => {
        ...
        overlay.innerHTML = `
          <div style="width:min(520px,100%);max-height:86vh;overflow:auto;background:#fff;border-radius:20px;box-shadow:0 24px 70px rgba(15,23,42,.28);padding:20px;">
            <h3 style="margin:0 0 8px;font-size:20px;color:#0f172a;">${escapeHtml((String(providerLabel || '').includes('카카오') ? 'SitePass 카카오 간편가입 약관' : (String(providerLabel || '').includes('네이버') ? 'SitePass 네이버 간편가입 약관' : 'SitePass 소셜 간편가입 약관')))}</h3>
            <p style="margin:0 0 14px;font-size:13px;line-height:1.55;color:#475569;">${escapeHtml(providerLabel || '소셜')} 공식 약관이 아니라 <b>SitePass 서비스 가입 약관</b>입니다. ${escapeHtml(providerLabel || '소셜')} 계정은 SitePass 로그인 식별에 사용합니다. 처음 이용하는 경우에만 아래 SitePass 필수 약관에 한 번 동의하면 회원가입과 로그인이 함께 완료됩니다. 다음부터는 같은 소셜 계정으로 바로 로그인됩니다. 다른 일반·카카오·네이버 계정과 자동으로 합치지 않습니다.</p>
            ...
            <label style="display:flex;gap:8px;align-items:flex-start;padding:9px 10px;border:1px solid #e2e8f0;border-radius:12px;margin-bottom:8px;font-size:13px;line-height:1.35;color:#334155;box-sizing:border-box;"><input class="socialRequiredTerm" type="checkbox" data-term="documentShare" style="width:16px;min-width:16px;max-width:16px;height:16px;min-height:16px;max-height:16px;flex:0 0 16px;margin:2px 0 0;padding:0;accent-color:#2563eb;box-sizing:border-box;" /> <span style="display:block;line-height:1.4;word-break:keep-all;overflow-wrap:break-word;"><b>[필수] 서류 보관·공유</b><br>현장 제출용 서류와 QR/링크가 SitePass에 저장되고 선택한 상대방에게 공유될 수 있음을 확인합니다.</span></label>
            <label style="display:flex;gap:8px;align-items:flex-start;padding:9px 10px;border:1px solid #e2e8f0;border-radius:12px;margin-bottom:8px;font-size:13px;line-height:1.35;color:#334155;box-sizing:border-box;"><input class="socialRequiredTerm" type="checkbox" data-term="thirdPartyDocumentHandling" style="width:16px;min-width:16px;max-width:16px;height:16px;min-height:16px;max-height:16px;flex:0 0 16px;margin:2px 0 0;padding:0;accent-color:#2563eb;box-sizing:border-box;" /> <span style="display:block;line-height:1.4;word-break:keep-all;overflow-wrap:break-word;"><b>[필수] 제3자 서류 처리 확인</b><br>타인의 서류를 등록·공유하는 경우 필요한 권한 또는 동의를 확보하고 목적 범위 안에서 이용할 책임이 있음을 확인합니다.</span></label>
            <label style="display:flex;gap:8px;align-items:flex-start;padding:9px 10px;border:1px solid #e2e8f0;border-radius:12px;margin-bottom:8px;font-size:13px;line-height:1.35;color:#334155;box-sizing:border-box;"><input class="socialRequiredTerm" type="checkbox" data-term="identity" style="width:16px;min-width:16px;max-width:16px;height:16px;min-height:16px;max-height:16px;flex:0 0 16px;margin:2px 0 0;padding:0;accent-color:#2563eb;box-sizing:border-box;" /> <span style="display:block;line-height:1.4;word-break:keep-all;overflow-wrap:break-word;"><b>[필수] 본인확인·계정찾기</b><br>향후 본인확인과 내 계정 찾기에 필요한 휴대폰 인증정보를 해당 목적에 이용하는 데 동의합니다.</span></label>
            <label style="display:flex;gap:8px;align-items:flex-start;padding:9px 10px;border:1px solid #e2e8f0;border-radius:12px;margin-bottom:8px;font-size:13px;line-height:1.35;color:#334155;box-sizing:border-box;"><input class="socialRequiredTerm" type="checkbox" data-term="sms" style="width:16px;min-width:16px;max-width:16px;height:16px;min-height:16px;max-height:16px;flex:0 0 16px;margin:2px 0 0;padding:0;accent-color:#2563eb;box-sizing:border-box;" /> <span style="display:block;line-height:1.4;word-break:keep-all;overflow-wrap:break-word;"><b>[필수] 인증문자 수신</b><br>회원가입·본인확인 과정에서 필요한 인증문자를 받을 수 있음에 동의합니다. 광고성 문자 수신 동의와는 별개입니다.</span></label>
            ...
            <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px;">
              ...
              <button id="socialTermsOk" type="button" style="border:0;background:#2563eb;color:#fff;border-radius:12px;padding:10px 16px;font-weight:800;">약관 동의하고 바로 시작</button>
            </div>
          </div>`;
        ...
        overlay.querySelector('#socialTermsOk').addEventListener('click', () => {
          ...
          cleanup({
            ...
            documentShare:true,
            thirdPartyDocumentHandling:true,
            identity:true,
            sms:true,
            responsibility:true,
    ...
    function getSignupAgreements() {
      ...
      return {
        ...
        // v23.7.428: 회원가입 기본 약관과 서류 등록/공유 약관을 분리합니다.
