// SitePass v23.7.335 - speed optimized medium chunk (app-core-auth-speed 04/04)
// ---- merged from app-core-auth-16.js ----
// SitePass v23.7.335 - app-core-auth finer split (16/19)
...
// SitePass v23.7.335 - app-core-auth finer split (17/19)
function renderAdminContactManager() {
    ...
    function showScreen(id, options) {
      if (sitePassCurrentScreenId === 'registerScreen' && id !== 'registerScreen') {
        // v23.7.335: 등록완료 후 보관함으로 즉시 이동할 때는
      ...
      if (id === 'homeScreen') {
        ...
        // v23.7.335: PC에서 등록한 보관함이 휴대폰에 보이지 않는 경우를 줄이기 위해
        // 로그인 후 홈 진입 시에도 일반회원 내 서버 보관함을 가볍게 동기화합니다.
        if (typeof sitePassAutoSyncLocalVisibleEquipmentItemsToSupabase === 'function' && isMemberLoggedIn() && !isAdminLoggedIn()) {
          setTimeout(function(){ try { sitePassAutoSyncLocalVisibleEquipmentItemsToSupabase('homeScreen'); } catch (e) {} }, 180);
        }
        if (typeof syncSupabaseMyEquipmentItems === 'function' && isMemberLoggedIn() && !isAdminLoggedIn()) {
          setTimeout(function(){ try { syncSupabaseMyEquipmentItems(true); } catch (e) {} }, 900);
        }
      }
      ...
      if (id === 'listScreen') {
        ...
        if (typeof sitePassAutoSyncLocalVisibleEquipmentItemsToSupabase === 'function' && isMemberLoggedIn() && !isAdminLoggedIn()) {
          setTimeout(function(){ try { sitePassAutoSyncLocalVisibleEquipmentItemsToSupabase('listScreen'); } catch (e) {} }, 160);
        }
        if (typeof syncSupabaseMyEquipmentItems === 'function' && isMemberLoggedIn() && !isAdminLoggedIn()) {
          setTimeout(function(){ try { syncSupabaseMyEquipmentItems(true); } catch (e) {} }, 1100);
        }
      }
...
// SitePass v23.7.335 - app-core-auth finer split (18/19)
...
// SitePass v23.7.335 - app-core-auth finer split (19/19)
function setPersonAuthStatus(kind, text, mode) {
    ...
    function verifyPersonAuth(kind) {
      ...
      // v23.7.335: 인부는 인증 완료 후 사용자가 보통/특수 추가 버튼을 다시 누르지 않도록
