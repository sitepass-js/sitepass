// SitePass v23.7.345 - speed optimized medium chunk (app-register-share-payment-speed 01/04)
// ---- merged from app-register-share-payment-01.js ----
// SitePass v23.7.345 - app-register-share-payment finer split (01/15)
// SitePass v23.7.345 - app.bundle.js remaining split (03 register/share/payment)
    ...
    // v23.7.345: 수신자 QR/1일 링크 화면은 회원이 실제 첨부한 서류만 보여줍니다.
    ...
    function hasRealPageAttachment(page) {
      ...
      // v23.7.345: 수신자 화면에서는 fileName/status만 남은 예전 껍데기 자료를 첨부로 보지 않습니다.
    ...
    function confirmLeaveRegistrationIfNeeded(targetScreenId) {
      // v23.7.345: 사진/파일 첨부 후 등록완료 전에 홈/보관함/뒤로가기 등으로 나가면
...
// SitePass v23.7.345 - app-register-share-payment finer split (02/15)
...
// SitePass v23.7.345 - app-register-share-payment finer split (03/15)
...
// SitePass v23.7.345 - app-register-share-payment finer split (04/15)
...
포함: ${bundleMeta.includedGroupNames.join(', ')}${editSavedLightNote}`);
      ...
      if (window.SITEPASS_TEST_NO_PAYMENT_MODE) {
        // v23.7.345: 테스트 등록완료는 Supabase/전체 보관함 렌더링/결제대기 처리를 기다리지 않고
