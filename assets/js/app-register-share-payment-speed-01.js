// SitePass v23.7.320 - speed optimized medium chunk (app-register-share-payment-speed 01/04)
// ---- merged from app-register-share-payment-01.js ----
// SitePass v23.7.320 - app-register-share-payment finer split (01/15)
// SitePass v23.7.320 - app.bundle.js remaining split (03 register/share/payment)
    ...
    function confirmLeaveRegistrationIfNeeded(targetScreenId) {
      // v23.7.320: 사진/파일 첨부 후 등록완료 전에 홈/보관함/뒤로가기 등으로 나가면
...
// SitePass v23.7.320 - app-register-share-payment finer split (02/15)
...
// SitePass v23.7.320 - app-register-share-payment finer split (03/15)
...
// SitePass v23.7.320 - app-register-share-payment finer split (04/15)
...
포함: ${bundleMeta.includedGroupNames.join(', ')}${editSavedLightNote}`);
      ...
      if (window.SITEPASS_TEST_NO_PAYMENT_MODE) {
        // v23.7.320: 테스트 기간에는 결제대기 상태를 localStorage/sessionStorage에 남기지 않습니다.
        ...
        // v23.7.320: 버튼 클릭 핸들러가 완료 처리 전체를 붙잡지 않도록 다음 틱에서 빠른 완료를 실행합니다.
        setTimeout(function(){
          try { completePendingRegistrationPayment('test-free'); }
          catch (e) { console.error('테스트 등록 빠른 완료 실행 실패:', e); alert('등록완료 처리 중 오류가 발생했습니다. 콘솔 오류를 확인해주세요.'); }
        }, 0);
        return;
      }
