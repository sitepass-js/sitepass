...
function renderAdmin() {
      ...
      try {
      if (!sitePassEquipmentSyncing && Date.now() - Number(window.sitePassAdminEquipmentSyncAttemptAtV549 || 0) > 120000 && (!sitePassEquipmentSyncedAt || Date.now() - sitePassEquipmentSyncedAt > 120000)) {
...
function expireUnpaidPaymentTestData() {
    ...
    function checkHash() {
      ...
      if (hash.startsWith('#manager=') || /(?:^\?|&)manager=/.test(search)) {
        ...
        if (parsed && parsed.code) {
          ...
          target.searchParams.set('v', '23.7.549-test');
    ...
    async function bootSitePassApp() {
      try {
        ...
        updateHomeInstallButtonState();
        // v23.7.549-test: 담당자 링크는 head 단계에서 recipient.html로 이동합니다.
        ...
        if (!handledOAuth && !checkHash()) {
          ...
          if (initialScreen === 'contactScreen') {
            // v23.7.549-test: 새로고침 시 사용자가 보고 있던 알림방을 그대로 복원합니다.
            setTimeout(function(){
              try {
