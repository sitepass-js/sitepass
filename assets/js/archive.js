// SitePass v23.7.348 - 보관함/장비서류 목록/관리자 집계 삭제반영 보정 파일
...
function isArchiveDeletedItem(item) {
  ...
  // v23.7.348: 일반회원 보관함은 PC/휴대폰 모두 서버목록만 기준입니다.
...
async function markArchiveCodeDeletedOnServer(code) {
  ...
  try {
    // v23.7.348: 기존 RPC 호출은 p_code와 code를 같이 보내 PostgREST 함수 매칭이 실패할 수 있었습니다.
