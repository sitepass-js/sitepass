// SitePass v23.7.458 - 서버 회원 없음 로그인 차단 가드
(function(){
  'use strict';
  ...
  function dbMissingSignupMessage(){
    return 'DB에 회원정보가 없습니다.\n회원가입을 새로 진행해주세요.';
  }
  ...
  function showDbMissingSignupBanner(){
    try {
      ...
      if (!panel) return;
      var old = document.getElementById('sitepassDbMissingSignupNotice456');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      var box = document.createElement('div');
      box.id = 'sitepassDbMissingSignupNotice456';
  ...
  function showDbMissingSignupPopup(message){
    var text = message || dbMissingSignupMessage();
  ...
  async function invalidateDeletedMember(message){
    ...
    if (message && t - lastAlertAt > 2500) {
      lastAlertAt = t;
      showDbMissingSignupPopup(message);
    }
    return false;
  }
  ...
  function wrapShowScreen(){
    if (window.__sitepassShowScreen458Wrapped || typeof window.showScreen !== 'function') return;
    window.__sitepassShowScreen458Wrapped = true;
  ...
  function wrapMemberLogin(){
    if (window.__sitepassLogin458Wrapped || typeof window.submitSitePassLoginTest !== 'function') return;
    window.__sitepassLogin458Wrapped = true;
    ...
    window.submitSitePassLoginTest = async function(){
      ...
      try {
        if (loginId && !(typeof window.isSuperAdminLoginId === 'function' && window.isSuperAdminLoginId(loginId))) {
          ...
          if (!serverMember) {
            await invalidateDeletedMember('DB에 회원정보가 없습니다.\n회원가입을 새로 진행해주세요.');
            return false;
          }
        }
      } catch(e) {
  ...
  function wrapSocialFallbackLogin(){
    if (window.__sitepassSocialLogin458Wrapped || typeof window.submitSocialLoginTest !== 'function') return;
    window.__sitepassSocialLogin458Wrapped = true;
