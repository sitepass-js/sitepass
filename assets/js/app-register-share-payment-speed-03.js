// SitePass v23.7.545-test - 회원 상세보기·공유 준비 (담당자 렌더링은 recipient.html 전용) (03/04)
...
function shareOneListItemEmail(code) {
    ...
    function applyManagerShareRecoveredUrlV497(obj, url) {
      ...
      // v23.7.545-test: data/blob 원본은 Storage 재업로드에 필요한 유일한 원본일 수 있습니다.
    ...
    function getManagerShareCandidateScoreV497(item) {
      ...
      // v23.7.545-test: 휴대폰에 남은 data/blob 원본을 오래된 404 URL보다 우선합니다.
...
function normalizePhoneForShare(phone) {
    ...
    // v23.7.545-test: 장비 상세보기는 v520의 단순 흐름을 기준으로 복원합니다.
...
function renderDocExpiryStrip(doc) {
    ...
    async function openManagerPublicView(code, expireAt, sig, options) {
      ...
      // v23.7.545-test: 로그인 직후 캐시에 서류목록이 아직 없으면 진행 중인 첫 서버동기화를
      ...
      // v23.7.545-test: 회원이 보관함에서 여는 링크화면은 수신자 공유링크와 분리합니다.
      ...
      url.searchParams.set('v', '23.7.545-test');
