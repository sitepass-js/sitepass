/* SitePass v23.7.470-test - 장비 → 기사 → 인부 단계별 등록 */
(function(){
  'use strict';

  const STEP_KEY = 'sitepass_v470_registration_wizard_step';
  ...
  function stepForStage(stage){
    ...
    if (n === 2) return document.getElementById('includeDriverDocs')?.checked ? 'driver' : 'driver-choice';
    return document.getElementById('includeWorkerDocs')?.checked ? 'worker' : 'worker-choice';
  }
  function navigateStage(stage){
    const targetStage = Math.max(1, Math.min(3, Number(stage || 1)));
    const currentStage = stageNumberForStep(currentStep);
    ...
    /* 이전 단계는 언제든 돌아갈 수 있습니다. */
    if (targetStage < currentStage) {
      applyStep(stepForStage(targetStage));
      return;
    }

    /* 다음 단계로 이동할 때는 앞 단계의 필수내용을 먼저 확인합니다. */
    if (currentStage === 1) {
      if (!validateBasic() || !validateStaticGroup('equipment')) return;
      saveDraft();
      applyStep(stepForStage(2));
      if (targetStage === 3) {
        setTimeout(function(){ alert('기사 있음 또는 기사 없음을 먼저 선택한 뒤 인부서류 단계로 이동해주세요.'); }, 80);
      }
      return;
    }

    if (currentStep === 'driver-choice') {
      alert('기사 있음 또는 기사 없음을 먼저 선택해주세요.');
      return;
    }
