// SitePass v23.7.462-test - 내정보 비밀번호 재확인 / 가입 인증정보 표시 / 간소화 수정 UI
(function () {
  'use strict';

  var PROFILE_KEY = 'sitepass_signup_profiles_v462';
  window.__sitepassMyAccountVerified462 = false;
  ...
  function readProfiles() {
    try {
      var value = JSON.parse(localStorage.getItem(PROFILE_KEY) || '{}');
  ...
  async function refreshProfileFromServer(ctx) {
    if (!ctx || !ctx.loginId || typeof window.sitePassLookupLoginState460 !== 'function') return;
    try {
      var state = await window.sitePassLookupLoginState460(ctx.loginId);
      ...
      var betterName = serverName && serverNameKey !== loginKey && (currentNameKey === loginKey || !ctx.name || ctx.name === 'SitePass 회원');
      var betterPhone = !!serverPhone && !digits(ctx.phone);
      if (!betterName && !betterPhone) return;
      ...
      if (window.__sitepassMyAccountVerified462) {
  ...
  function openMyAccountScreen462() {
    var ctx = resolveContext();
    if (!ctx) {
  ...
  function confirmEntryPassword() {
    ...
    window.__sitepassMyAccountVerified462 = true;
    try { myAccountPasswordVerified = true; } catch (e) {}
    hideGate();
    renderMyAccount();
  ...
  function openPhoneEdit() {
    if (!window.__sitepassMyAccountVerified462) return openMyAccountScreen462();
    var ctx = resolveContext();
  ...
  window.openMyAccountScreen = openMyAccountScreen462;
