// SitePass v23.7.463-test - 서버 회원 확인 + 기존 회원 프로필 보강 가드
(function(){
  'use strict';
  ...
  async function callLookupRpc(name, lookup){
    try {
      var res = await window.sitepassSupabase.rpc(name, {
      ...
      if (res && res.error) {
        ...
        if (/not found|could not find|schema cache|function/i.test(msg)) return result('unknown', null, res.error, name + '-missing');
        return result('unknown', null, res.error, name + '-error');
      }
      var data = res ? res.data : null;
      if (data && data.found === true && data.row) return result(isActiveMemberRow(data.row) ? 'found' : 'missing', data.row, null, name);
      if (data && data.found === false) return result('missing', null, null, name);
      if (data && data.login_id) return result(isActiveMemberRow(data) ? 'found' : 'missing', data, null, name);
      return result('unknown', null, null, name + '-ambiguous');
    } catch(e) {
      return result('unknown', null, e, name + '-exception');
    }
  }
  ...
  async function rpcLookup(lookup){
    try {
      ...
      var current = await callLookupRpc('sitepass_lookup_member_public_v463', lookup);
      if (current.state !== 'unknown') return current;
      var legacy = await callLookupRpc('sitepass_lookup_member_public_v460', lookup);
      return legacy;
    } catch(e) {
  ...
  async function directQuery(buildQuery){
    try {
      ...
      var base = window.sitepassSupabase.from('sitepass_members').select('login_id, name, provider_id, phone, email, auth_user_id, signup_method, role, status, plan_type, terms_agreed_at, created_at, updated_at').limit(5);
  ...
  async function verifyCurrentMember(reason, silent){
    ...
    if (state.state === 'missing' && isNewSignupGraceActive(member)) {
      console.warn('[SitePass v463] 신규가입 직후 서버 저장 완료 전 조회여서 로그인은 유지하고 다시 확인합니다.');
    ...
    console.warn('[SitePass v463] 서버 회원 확인이 권한/RLS/네트워크 문제로 확정되지 않아 기존 로그인은 유지합니다.', state.source, state.error && (state.error.message || state.error));
    return true;
  }
  ...
  function wrapMemberLogin(){
    ...
    window.submitSitePassLoginTest = async function(){
      ...
      if (loginId && !(typeof window.isSuperAdminLoginId === 'function' && window.isSuperAdminLoginId(loginId))) {
        ...
        if (state.state === 'unknown') console.warn('[SitePass v463] 로그인 전 서버 회원 확인은 확정되지 않았습니다. 로컬 계정 확인을 계속합니다.');
      }
      return prev.apply(this, arguments);
    };
  }

  window.sitePassMarkNewSignupGrace463 = markNewSignupGrace;
  window.sitePassLookupLoginState463 = lookupLoginState;
  window.sitePassLookupLocalMemberState463 = lookupLocalState;
  window.sitePassFetchActiveMemberForLogin463 = fetchActiveMemberForLogin;
  window.sitePassFetchActiveMemberForLocalMember463 = fetchActiveMemberForLocalMember;
  window.sitePassVerifyCurrentMember463 = verifyCurrentMember;
  window.sitePassInvalidateDeletedMemberSession463 = invalidateDeletedMember;
  // 기존 코드 호환
