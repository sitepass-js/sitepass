// SitePass v23.7.279 - 장비등록 목록/관리자수 반영 보정
...
const STORAGE_KEY = 'sitePass_v23_7_7_update_original_corrected';
    ...
    // v23.7.279 - 장비등록 저장 시 소유회원이 비어 있으면 가능한 회원정보로 보강합니다.
...
포함: ${bundleMeta.includedGroupNames.join(', ')}${editSavedLightNote}`);
      ...
      // v23.7.279: 장비등록 직후 결제대기 상태라도 보관함/관리자 장비수에 보이게 먼저 저장합니다.
...
${escapePlainTextForAlert(paidItem.equipmentName || '장비')} QR링크가 생성되고 보관함에 저장되었습니다.${paymentSavedLightNote}`);
    ...
    function mergePendingRegistrationIntoItems(list) {
      ...
      try {
        ...
        if (pendingItem && pendingItem.code && !safeList.some(item => String(item && item.code || '') === String(pendingItem.code || ''))) {
          safeList.unshift(pendingItem);
        }
      } catch (e) {}
      return safeList;
    }
    ...
    function getItems() {
      try {
        ...
        if (Array.isArray(current) && current.length) return mergePendingRegistrationIntoItems(current);
        const previousV2376 = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY_7 || '') || '[]');
        if (Array.isArray(previousV2376) && previousV2376.length) return mergePendingRegistrationIntoItems(previousV2376);
        const previousV2375 = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY_6 || '') || '[]');
        if (Array.isArray(previousV2375) && previousV2375.length) return mergePendingRegistrationIntoItems(previousV2375);
        const previousV2374 = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY_5 || '') || '[]');
        if (Array.isArray(previousV2374) && previousV2374.length) return mergePendingRegistrationIntoItems(previousV2374);
        const previousV2373 = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY_4 || '') || '[]');
        if (Array.isArray(previousV2373) && previousV2373.length) return mergePendingRegistrationIntoItems(previousV2373);
        const previousV2372 = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY_3 || '') || '[]');
        if (Array.isArray(previousV2372) && previousV2372.length) return mergePendingRegistrationIntoItems(previousV2372);
        const previousV237 = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY_2 || '') || '[]');
        if (Array.isArray(previousV237) && previousV237.length) return mergePendingRegistrationIntoItems(previousV237);
        const previous = JSON.parse(localStorage.getItem(PREV_STORAGE_KEY || '') || '[]');
        if (Array.isArray(previous) && previous.length) return mergePendingRegistrationIntoItems(previous);
        return mergePendingRegistrationIntoItems([]);
      }
    ...
    const SITEPASS_APP_VERSION = (window.SITEPASS_DB_CONFIG && window.SITEPASS_DB_CONFIG.appVersion) || 'v23.7.279';
