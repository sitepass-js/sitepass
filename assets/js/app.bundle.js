// SitePass v23.7.253 split step 1 - 기존 index.html의 전체 기능 JavaScript 묶음
// 다음 단계에서 이 파일을 auth-kakao.js, auth-naver.js, terms.js, admin-members.js 등으로 나눌 예정입니다.
const STORAGE_KEY = 'sitePass_v23_7_7_update_original_corrected';
    ...
    async function startSupabaseKakaoOAuth(mode) {
      try {
        ...
        await signOutSupabaseAuthQuietly();
        setSessionValue(SITEPASS_OAUTH_PENDING_KEY, JSON.stringify({
          provider:'kakao',
          mode: mode || 'login',
    ...
    // v23.7.208 - 휴대폰 inline onclick 안전 연결
    ...
    async function handleSupabaseKakaoOAuthReturn() {
      ...
      try {
        ...
        try { removeSessionValue(SITEPASS_OAUTH_PENDING_KEY); } catch (ignore) {}
        if (hasSupabaseKakaoReturnParams()) {
    ...
    function getSignupAgreements() {
      ...
      return {
        ...
        documentStorage: !!document.getElementById('agreeDocumentTerms')?.checked,
        sensitive: !!document.getElementById('agreeSensitiveTerms')?.checked,
    ...
    function resetSitePassSignupPhoneAuth() {
      ...
      if (status) status.textContent = '이름, 주민번호, 휴대폰번호, 통신사를 입력한 뒤 인증요청을 눌러주세요. 주민번호는 840507-1****** 형식입니다. 임시 인증번호는 123456입니다.';
    }
    ...
    function sendSitePassSignupPhoneCodeTest() {
      ...
      if (!/^\d{6}$/.test(identity.birth6) || !/^[1-8]$/.test(identity.genderDigit)) {
        alert('주민번호는 840507-1****** 형식으로 입력해주세요.');
        return;
      }
    ...
    function sendPersonAuthCode(kind) {
      ...
      if (!values.birth6 || !/^\d{6}$/.test(values.birth6) || !values.genderDigit || !/^[1-8]$/.test(values.genderDigit)) { alert((kind === 'driver' ? '기사' : '인부') + ' 주민번호는 840507-1****** 형식으로 입력해주세요.'); values.panel.querySelector('[data-person-auth-jumin]')?.focus(); return; }
    ...
    function renderDriverAuthPanel() {
      return '<div class="person-auth-panel" data-person-auth-panel="driver" data-auth-code-sent="false" data-auth-verified="false">' +
        ...
        '<div class="person-auth-grid"><input type="text" data-person-auth-name placeholder="기사 이름" autocomplete="off" /><input type="text" data-person-auth-jumin placeholder="주민번호 예: 840507-1******" maxlength="14" inputmode="text" autocomplete="off" onblur="formatPersonAuthJuminDisplay(this)" /></div>' +
    ...
    function renderWorkerPeopleSection() {
      return '<div class="worker-control-box">' +
        ...
        '<div class="person-auth-panel" data-person-auth-panel="worker" data-auth-code-sent="false" data-pending-verified="false">' +
          ...
          '<div class="person-auth-grid three"><select data-person-auth-type><option value="normal">보통인부</option><option value="special">특수인부</option></select><input type="text" data-person-auth-name placeholder="인부 이름" autocomplete="off" /><input type="text" data-person-auth-jumin placeholder="주민번호 예: 840507-1******" maxlength="14" inputmode="text" autocomplete="off" onblur="formatPersonAuthJuminDisplay(this)" /></div>' +
    ...
    function requirePaymentOwnerVerification(actionLabel) {
      ...
      const methodRaw = prompt(label + ' 본인확인\n\n결제수단을 선택해주세요.\n1. 본인 명의 카드결제\n2. 본인 명의 휴대폰결제\n3. 계좌이체/무통장 별도확인\n\n번호를 입력하세요.', '1');
      if (methodRaw === null) return false;
      const method = String(methodRaw).trim();
      if (method === '3') {
        const ok = prompt('계좌이체는 카드/휴대폰 명의확인과 별도로 PG/은행 인증으로 확인합니다.\n\n테스트 확인번호 1234를 입력하면 계속 진행합니다.');
        if (ok === null) return false;
        if (String(ok).trim() !== '1234') { alert('확인번호가 맞지 않습니다.'); return false; }
        return true;
      }
      if (method !== '1' && method !== '2') {
        alert('결제수단은 1, 2, 3 중에서 선택해주세요.');
        return false;
      }
      const methodLabel = method === '1' ? '카드 명의자' : '휴대폰 결제 명의자';
      const name = prompt(methodLabel + ' 이름을 입력해주세요.', member.name || '');
      if (name === null) return false;
      const juminInput = prompt(methodLabel + ' 주민번호를 입력해주세요.\n예: 840507-1******', member.birth6 && member.genderDigit ? member.birth6 + '-' + member.genderDigit + '******' : '');
      if (juminInput === null) return false;
      const parsedJumin = parseMaskedJuminText(juminInput, member.birth6 || '', member.genderDigit || '');
      const birth6 = parsedJumin.birth6;
      const genderDigit = parsedJumin.genderDigit;
      const carrier = prompt('통신사를 입력해주세요.\n휴대폰결제는 이 통신사와 휴대폰번호가 본인 명의인지 확인해야 합니다.', member.carrier || '');
      if (carrier === null) return false;
      const phone = prompt(methodLabel + ' 휴대폰번호를 입력해주세요.', member.phone || '');
      if (phone === null) return false;
      if (!name.trim() || !/^\d{6}$/.test(String(birth6).trim()) || !/^[1-8]$/.test(String(genderDigit).trim()) || !carrier.trim() || !/^01[0-9]-?[0-9]{3,4}-?[0-9]{4}$/.test(String(phone).trim())) {
        alert('이름, 주민번호, 통신사, 휴대폰번호를 정확히 입력해주세요.');
        return false;
      }
      if (method === '2' && member.phone && normalizePhoneDigits(phone) !== normalizePhoneDigits(member.phone)) {
        alert('휴대폰결제는 가입자 본인 휴대폰번호와 일치해야 합니다.\n가입 휴대폰번호: ' + member.phone);
        return false;
      }
      const code = prompt(methodLabel + ' 본인확인 인증번호 6자리를 입력해주세요.\n현재 테스트 인증번호는 123456입니다.');
      if (code === null) return false;
      if (String(code).trim() !== '123456') {
        alert('인증번호가 맞지 않습니다. 현재 테스트 인증번호는 123456입니다.');
        return false;
      }
      alert(label + ' 본인확인이 완료되었습니다.\n정식 서비스에서는 카드사/통신사/결제대행사 본인확인 결과값을 서버에 저장합니다.');
      return true;
    }
    ...
    function saveEquipment() {
      const equipmentNoEl = document.getElementById('equipmentNo');
      const equipmentNameEl = document.getElementById('equipmentName');
      const equipmentNo = equipmentNoEl.value.trim();
      const equipmentName = equipmentNameEl.value.trim();
...
이제 결제방법 화면으로 이동합니다.
결제를 완료하면 QR링크가 생성되고 보관함에 저장됩니다.`);
      openPendingRegistrationPaymentScreen(pending);
    }

    function completePendingRegistrationPayment(plan) {
      const pending = getPendingRegistration();
      if (!pending || !pending.item) { alert('결제 대기 중인 등록서류가 없습니다.'); return; }
      if (!requirePaymentOwnerVerification('등록 결제')) return;
      const items = getItems();
      const item = pending.item;
      if (items.some(x => String(x.code || '') === String(item.code || ''))) {
        alert('이미 같은 장비번호의 서류함이 저장되어 있습니다. 보관함을 확인해주세요.');
      ...
      if (paidItem.bundleMeta) paidItem.bundleMeta.paymentText = info.planText + ' 결제완료';
      items.unshift(paidItem);
      let saved = setItems(items);
      let savedLight = false;
      if (!saved) {
        const lightItems = items.map(makeStorageLightItem);
...
${escapePlainTextForAlert(paidItem.equipmentName || '장비')} QR링크가 생성되고 보관함에 저장되었습니다.${paymentSavedLightNote}`);
    ...
    async function loadManagerShareItemFromSupabase(code, sig) {
      const client = getSitePassSupabaseClient();
      if (!client || !code || !sig) return { ok:false, message:'Supabase 연결 또는 링크 서명이 없습니다.' };
      try {
        ...
        if (expiresAt && Date.now() > expiresAt) return { ok:false, expired:true, expiresAt };
        const item = data.item_data || null;
        if (!item || !item.code) return { ok:false, message:'공유 데이터가 비어 있습니다.' };
        const cached = upsertSharedItemIntoLocalCache(item);
    ...
    function buildPrintHtml(item, pages, blockedPages) {
      ...
      const docsHtml = pages.map((page, index) => {
        const dateText = page.expireDate ? getExpiryPeriodLabel(page.doc) + ': ' + getDdayTextWithDays(page.expireDate) + ' / ' + page.expireDate : '';
        const extra = renderPlainExtra(page);
        const hasExtra = !!extra;
        const pageTitle = (item.equipmentNo ? item.equipmentNo + ' ' : '') + page.docTitle + ((page.pageNo && page.pageNo > 1 && page.totalPages > 1) ? ' - ' + page.pageNo + '페이지' : '');
    ...
    function setPendingRegistration(pending) {
      ...
      try { sessionStorage.setItem(PENDING_REGISTRATION_KEY, JSON.stringify(pending || null)); return true; } catch (e) {}
      try { localStorage.setItem(PENDING_REGISTRATION_KEY, JSON.stringify(pending || null)); return true; } catch (e) {}
    ...
    function renderPendingRegistrationPaymentBox() {
      ...
      box.classList.remove('hidden');
      box.innerHTML = '<b>결제 대기 중인 등록서류</b><br>' +
        escapeHtml(tierText) + ' · ' + escapeHtml(item.equipmentName || '장비명 없음') + ' / ' + escapeHtml(item.equipmentNo || '') + '<br>' +
        '결제를 완료하면 이 장비의 QR·담당자 링크가 바로 생성되고 보관함에 저장됩니다.';
    }
    ...
    function toggleAdminMemberDetail(memberId) {
      ...
      adminExpandedMemberId = adminExpandedMemberId === key ? '' : key;
      renderAdmin();
    }
    ...
    async function forceWithdrawMember(memberId) {
      ...
      adminServerMemberRows = removeRowsByMemberKeys(adminServerMemberRows, target);
