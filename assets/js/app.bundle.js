// SitePass v23.7.280 - 장비등록 목록/관리자수 반영 보정
...
const STORAGE_KEY = 'sitePass_v23_7_7_update_original_corrected';
    ...
    const SERVER_EQUIPMENT_CACHE_KEY = STORAGE_KEY + '_server_equipment_cache_v23_7_280';
    ...
    // v23.7.280 - 장비등록 저장 시 소유회원이 비어 있으면 가능한 회원정보로 보강합니다.
...
보관함에서 기존 장비를 확인하거나 다른 장비번호로 등록해주세요.`);
      ...
      if (oldItem) {
        ...
        try { saveEquipmentItemToSupabase(item, 'edit'); } catch (e) { console.warn('수정 장비 서버 저장 예약 실패:', e); }
...
포함: ${bundleMeta.includedGroupNames.join(', ')}${editSavedLightNote}`);
      ...
      // v23.7.280: 장비등록 직후 결제대기 상태라도 보관함/관리자 장비수에 보이게 먼저 저장합니다.
      ...
      try { saveEquipmentItemToSupabase(item, 'pending_registration'); } catch (e) { console.warn('결제대기 장비 서버 저장 예약 실패:', e); }
...
결제를 완료하면 QR링크가 생성되고 보관함에 저장됩니다.`);
    ...
    function completePendingRegistrationPayment(plan) {
      ...
      try { saveEquipmentItemToSupabase(paidItem, 'payment_completed'); } catch (e) { console.warn('결제완료 장비 서버 저장 예약 실패:', e); }
      try { syncSupabaseEquipmentItems(true); } catch (e) {}
...
${escapePlainTextForAlert(paidItem.equipmentName || '장비')} QR링크가 생성되고 보관함에 저장되었습니다.${paymentSavedLightNote}`);
    ...
    async function saveEquipmentItemToSupabase(item, reason) {
      ...
      if (!supabaseApi || !supabaseApi.upsert) return { skipped:true, error:'Supabase API 연결 없음' };
      const row = buildSupabaseEquipmentRow(item, reason);
      if (!row) return { skipped:true, error:'장비 row 없음' };
      try {
        const { error } = await supabaseApi.upsert('sitepass_equipment_items', row, { onConflict:'code' });
        if (error) {
          console.warn('Supabase 장비 저장 실패:', error);
          sitePassEquipmentSyncMessage = '장비 서버저장 실패: ' + (error.message || error);
          return { ok:false, error };
        }
        const cache = getServerEquipmentCache();
        const merged = mergeEquipmentItemLists(cache, [normalizeSupabaseEquipmentRow(row)]);
        setServerEquipmentCache(merged);
        sitePassEquipmentSyncMessage = '장비 서버저장 완료';
        return { ok:true };
      } catch (e) {
    ...
    async function syncSupabaseEquipmentItems(silent) {
      const supabaseApi = window.SitePassSupabaseApi;
      if (!supabaseApi || !supabaseApi.select || sitePassEquipmentSyncing) return { skipped:true };
      sitePassEquipmentSyncing = true;
      try {
        const { data, error } = await supabaseApi.select('sitepass_equipment_items', '*', function(query){
          return query.eq('is_deleted', false).order('updated_at', { ascending:false }).limit(1000);
        });
        if (error) {
          sitePassEquipmentSyncMessage = '장비 서버목록 불러오기 실패: ' + (error.message || error);
          if (!silent) alert(sitePassEquipmentSyncMessage);
          return { ok:false, error };
        }
        const rows = Array.isArray(data) ? data : [];
        const items = rows.map(normalizeSupabaseEquipmentRow).filter(Boolean);
        setServerEquipmentCache(items);
        sitePassEquipmentSyncedAt = Date.now();
        sitePassEquipmentSyncMessage = '서버 장비목록 동기화: ' + items.length + '대';
    ...
    const SITEPASS_APP_VERSION = (window.SITEPASS_DB_CONFIG && window.SITEPASS_DB_CONFIG.appVersion) || 'v23.7.280';
