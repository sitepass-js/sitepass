// SitePass v23.7.309 - speed optimized medium chunk (app-register-share-payment-speed 02/04)
// ---- merged from app-register-share-payment-05.js ----
// SitePass v23.7.309 - app-register-share-payment finer split (05/15)
async function completePendingRegistrationPayment(plan) {
      ...
      if (window.SITEPASS_TEST_NO_PAYMENT_MODE) {
        // v23.7.309: 테스트 완료 저장 중에는 더 이상 결제대기 자료가 getItems()/동기화에 섞이지 않게 먼저 정리합니다.
      ...
      if (!window.SITEPASS_TEST_NO_PAYMENT_MODE) {
        ...
        // v23.7.309: 테스트 등록완료 직후 전체 서버목록 RPC/SELECT가 timeout/RLS로 실패하면
...
// SitePass v23.7.309 - app-register-share-payment finer split (06/15)
function resetForm(clearEdit = true) {
    ...
    function setServerEquipmentCache(list) {
      ...
      try {
        ...
        // v23.7.309: 서버 장비 캐시는 보조 캐시라서, 용량 초과 때 원본 이미지/base64까지
    ...
    function shouldSyncSupabaseEquipmentItemsForCurrentContext() {
      // v23.7.309: 일반회원 화면에서 전체 장비목록 RPC/SELECT를 실행하면
...
// SitePass v23.7.309 - app-register-share-payment finer split (07/15)
function setItems(items) {
    ...
    function clearNonEssentialRegistrationStorageForSave() {
      // v23.7.309: 사진 등록 후 보관함 저장이 localStorage 용량 때문에 실패하지 않도록
    ...
    function getEssentialSitePassStorageKeysForSave() {
      const oauthPendingKey = (typeof SITEPASS_OAUTH_PENDING_KEY !== 'undefined' && SITEPASS_OAUTH_PENDING_KEY) ? SITEPASS_OAUTH_PENDING_KEY : (STORAGE_KEY + '_oauth_pending_v23_7_207');
      const keys = [STORAGE_KEY, MEMBER_STORAGE_KEY, CURRENT_MEMBER_KEY, ADMIN_SESSION_KEY, ADMIN_SESSION_KEY + '_role', ADMIN_SESSION_KEY + '_id', ADMIN_SESSION_KEY + '_name', ADMIN_ROLE_MAP_KEY, PWA_AUTO_MEMBER_KEY, QUICK_AUTH_KEY, SELECTED_PAYMENT_PLAN_KEY, oauthPendingKey];
      return new Set(keys.filter(Boolean).map(String));
    }

    function isSupabaseOrOAuthRuntimeStorageKey(key) {
      const k = String(key || '');
    ...
    function clearSitePassHeavyStorageForEmergencySave() {
      // v23.7.309: 현재 origin의 SitePass 구버전 사진/base64 캐시가 localStorage를 꽉 채우면
      ...
      keys.filter(Boolean).forEach(function(key) {
        ...
        if (keep.has(String(key)) || isSupabaseOrOAuthRuntimeStorageKey(key)) return;
...
// SitePass v23.7.309 - app-register-share-payment finer split (08/15)
